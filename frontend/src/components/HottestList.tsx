...
export default function HottestList() {
    ...
    useEffect(() => {
        const fetchHottest = async () => {
            try {
                // Now calling the single optimized endpoint
                const response = await articleService.getHottest();
        ...
        <div className="max-w-screen-2xl mx-auto p-6 lg:p-8">
            <div className="mb-12 border-l-[3px] border-[#189eac] pl-8">
                <div className="flex items-center gap-2 mb-2">
                    <Flame className="w-5 h-5 text-[#189eac]" />
                    <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#189eac]">Trending Analysis</span>
                </div>
                <h1 className="text-4xl font-bold text-white mb-3 tracking-tight">Hottest Coverage</h1>
                <p className="text-zinc-400 text-lg leading-relaxed max-w-3xl">
                    The stories below represent the highest volume of collective media attention.
                    These are the topics currently dominating the national conversation.
                </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-8">
                {articles.length === 0 ? (
                    <p className="col-span-full text-center text-zinc-500 text-lg">No trending topics found at the moment.</p>
                ) : (
                            ...
                            >
                                ...
                                <div className="p-5 flex-grow flex flex-col justify-between relative">
                                    ...
                                    <div
                                        ...
                                        View Comparison
                                        <div className="h-px flex-grow bg-zinc-800 group-hover:bg-[#189eac]/30 transition-colors" />
                                    </div>
                                </div>
                            </Link>
                        </div>
